...
def process_single_file(input_file: str, finalname: str, ncols: int | None = None):
    """
    Detect file type and convert to finalname.csv.
    """
    ...
    if filetype == "csv":
        ...
        ifcsv(input_file, str(output_tmp), ncols)
        renaming(str(output_tmp), finalname)

    elif filetype == "txt":
        if ncols is None:
            raise ValueError("ncols is required for TXT.")
        iftxt(input_file, str(output_tmp), ncols)
        renaming(str(output_tmp), finalname)

    elif filetype == "pssession":
        ifpssession(input_file, str(output_tmp), experiment_name=None)
        renaming(str(output_tmp), finalname)
...
def convert_all_pssession_in_folder(folder: str, experiment_name=None):
    ...
    for pss_file in pssession_files:
        csv_path = pss_file.with_suffix(".csv")
